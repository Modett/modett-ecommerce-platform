...
export class ShoppingCart {
  private constructor(
    ...
    if (!userId && !guestToken) {
      throw new Error("Cart must belong to either user or guest");
    }
  }
  ...
  transferToUser(userId: string): ShoppingCart {
    if (this.isUserCart()) {
      throw new Error("Cannot transfer user cart to another user");
    }
  ...
  mergeWith(otherCart: ShoppingCart): void {
    if (!this.isUserCart()) {
      throw new Error("Can only merge into user cart");
    }
