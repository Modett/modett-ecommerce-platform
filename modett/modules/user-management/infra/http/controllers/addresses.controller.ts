...
class AddressValidation {
  ...
  static sanitizeAddressData(data: AddAddressRequest | UpdateAddressRequest): any {
    return {
      ...data,
...
export class AddressesController {
  ...
  async addAddress(
    ...
    try {
      ...
      const missingFields = AddressValidation.validateRequiredFields(sanitizedData);
      ...
      if (!AddressValidation.validatePostalCode(sanitizedData.postalCode, sanitizedData.country)) {
        reply.status(HTTP_STATUS.BAD_REQUEST).send({
          success: false,
  ...
  async addCurrentUserAddress(
    ...
    try {
      ...
      this.logError('addCurrentUserAddress', error, { userId: (request as any).user?.userId });
      reply.status(HTTP_STATUS.INTERNAL_SERVER_ERROR).send({
        success: false,
  ...
  async getCurrentUserAddresses(
    ...
    try {
      ...
      this.logError('getCurrentUserAddresses', error, { userId: (request as any).user?.userId });
      reply.status(HTTP_STATUS.INTERNAL_SERVER_ERROR).send({
        success: false,
  ...
  async updateCurrentUserAddress(
    ...
    try {
      ...
      if (sanitizedData.postalCode && sanitizedData.country &&
          !AddressValidation.validatePostalCode(sanitizedData.postalCode, sanitizedData.country)) {
        reply.status(HTTP_STATUS.BAD_REQUEST).send({
          success: false,
          error: 'Invalid postal code format for the specified country',
          errors: ['postalCode']
      ...
      if (sanitizedData.phone && !AddressValidation.validatePhoneNumber(sanitizedData.phone)) {
        reply.status(HTTP_STATUS.BAD_REQUEST).send({
          success: false,
          error: 'Invalid phone number format',
          errors: ['phone']
