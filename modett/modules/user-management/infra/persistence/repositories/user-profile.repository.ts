...
export class UserProfileRepository implements IUserProfileRepository {
  ...
  async findByDefaultAddressId(addressId: string): Promise<UserProfile[]> {
    ...
    return profiles.map(profileData => UserProfile.fromDatabaseRow(this.mapPrismaToRow(profileData)));
  }

  async findByDefaultPaymentMethodId(paymentMethodId: string): Promise<UserProfile[]> {
    const profiles = await this.prisma.userProfile.findMany({
  ...
  async getProfileCompletionStats(): Promise<{
    ...
    const averageCompletion = total > 0 ? Math.round((complete / total) * 100) : 0;
  ...
  async updateDefaultAddress(userId: UserId, addressId: string | null): Promise<void> {
    await this.prisma.userProfile.update({
      where: { userId: userId.getValue() },
      data: {
  ...
  async updateDefaultPaymentMethod(userId: UserId, paymentMethodId: string | null): Promise<void> {
    await this.prisma.userProfile.update({
      where: { userId: userId.getValue() },
      data: {
