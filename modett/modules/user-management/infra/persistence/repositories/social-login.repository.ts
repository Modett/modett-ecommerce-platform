...
import { SocialLogin, SocialProvider } from "../../../domain/entities/social-login.entity";
...
export class SocialLoginRepository implements ISocialLoginRepository {
  ...
  async findByUserId(userId: UserId): Promise<SocialLogin[]> {
    ...
    return socialLogins.map(data => SocialLogin.fromDatabaseRow({
      social_id: data.id,
      user_id: data.userId,
      provider: data.provider,
  ...
  async findByProviderUserId(provider: SocialProvider, providerUserId: string): Promise<SocialLogin | null> {
  ...
  async findByUserIdAndProvider(userId: UserId, provider: SocialProvider): Promise<SocialLogin | null> {
    const socialLoginData = await this.prisma.socialLogin.findFirst({
      where: {
  ...
  async existsByProviderUserId(provider: SocialProvider, providerUserId: string): Promise<boolean> {
    const count = await this.prisma.socialLogin.count({
      where: {
        provider: provider.toString(),
        providerUserId,
  ...
  async findTokensExpiringSoon(minutesAhead: number = 30): Promise<SocialLogin[]> {
    // Since database doesn't store token expiry, return empty array
    // This would be implemented if token data was persisted
    return [];
  }
  ...
  async findByProviderEmail(provider: SocialProvider, email: string): Promise<SocialLogin[]> {
    // Since database doesn't store provider email, return empty array
    // This would be implemented if provider email was persisted
    return [];
  }

  async findDuplicateConnections(provider: SocialProvider, providerUserId: string): Promise<SocialLogin[]> {
    const socialLogins = await this.prisma.socialLogin.findMany({
      where: {
        provider: provider.toString(),
        providerUserId,
  ...
  async getSocialLoginStatsByProvider(): Promise<Array<{
    provider: string;
    total: number;
    active: number;
    inactive: number;
  ...
  async findRecentLogins(userId: UserId, hours: number = 24): Promise<SocialLogin[]> {
  ...
  async findStaleConnections(daysSinceLastUpdate: number): Promise<SocialLogin[]> {
  ...
  async cleanupInactiveConnections(daysSinceLastUpdate: number): Promise<number> {
    const cutoffDate = new Date(Date.now() - (daysSinceLastUpdate * 24 * 60 * 60 * 1000));
