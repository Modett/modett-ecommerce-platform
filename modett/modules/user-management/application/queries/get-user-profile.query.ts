...
import { ICommand, ICommandHandler, CommandResult } from '../commands/register-user.command';
...
export class GetUserProfileHandler implements IQueryHandler<GetUserProfileQuery, CommandResult<UserProfileResult>> {
  constructor(
    private readonly userProfileService: UserProfileService
  ) {}

  async handle(query: GetUserProfileQuery): Promise<CommandResult<UserProfileResult>> {
    try {
      // Validate query
      if (!query.userId) {
