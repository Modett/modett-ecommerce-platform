import { ISizeGuideRepository, SizeGuideQueryOptions, SizeGuideCountOptions } from '../../domain/repositories/size-guide.repository';
import { SizeGuide, SizeGuideId, Region, CreateSizeGuideData } from '../../domain/entities/size-guide.entity';
...
export class SizeGuideManagementService {
  ...
  async createSizeGuide(data: CreateSizeGuideData): Promise<SizeGuide> {
    ...
    if (data.category) {
      const existingGuide = await this.sizeGuideRepository.findByRegionAndCategory(data.region, data.category);
      if (existingGuide) {
        throw new Error(`Size guide already exists for region "${data.region}" and category "${data.category}"`);
      }
    }
  ...
  async getSizeGuideById(id: string): Promise<SizeGuide> {
    ...
    if (!sizeGuide) {
      throw new Error(`Size guide with ID "${id}" not found`);
    }

    return sizeGuide;
  }

  async getAllSizeGuides(options?: SizeGuideQueryOptions): Promise<SizeGuide[]> {
    return await this.sizeGuideRepository.findAll(options);
  }

  async getSizeGuidesByRegion(region: Region, options?: SizeGuideQueryOptions): Promise<SizeGuide[]> {
    return await this.sizeGuideRepository.findByRegion(region, options);
  }

  async getSizeGuidesByCategory(category: string, options?: SizeGuideQueryOptions): Promise<SizeGuide[]> {
    return await this.sizeGuideRepository.findByCategory(category, options);
  }

  async getSizeGuideByRegionAndCategory(region: Region, category: string): Promise<SizeGuide | null> {
    return await this.sizeGuideRepository.findByRegionAndCategory(region, category);
  }
  ...
  async updateSizeGuide(id: string, updates: {
    title?: string;
    bodyHtml?: string;
    region?: Region;
    category?: string;
    ...
    if (updates.region !== undefined) {
      ...
      const targetCategory = updates.category !== undefined ? updates.category : sizeGuide.getCategory();
      if (targetCategory) {
        const existingGuide = await this.sizeGuideRepository.findByRegionAndCategory(updates.region, targetCategory);
        if (existingGuide && !existingGuide.getId().equals(sizeGuide.getId())) {
          throw new Error(`Size guide already exists for region "${updates.region}" and category "${targetCategory}"`);
        }
      }
    ...
    if (updates.category !== undefined) {
      const targetRegion = updates.region !== undefined ? updates.region : sizeGuide.getRegion();
      if (updates.category) {
        const existingGuide = await this.sizeGuideRepository.findByRegionAndCategory(targetRegion, updates.category);
        if (existingGuide && !existingGuide.getId().equals(sizeGuide.getId())) {
          throw new Error(`Size guide already exists for region "${targetRegion}" and category "${updates.category}"`);
        }
      }
  ...
  async createRegionalSizeGuide(region: Region, data: Omit<CreateSizeGuideData, 'region'>): Promise<SizeGuide> {
  ...
  async getSizeGuidesByRegions(regions: Region[], options?: SizeGuideQueryOptions): Promise<SizeGuide[]> {
  ...
  async createCategorySizeGuide(category: string, region: Region, data: Omit<CreateSizeGuideData, 'category' | 'region'>): Promise<SizeGuide> {
  ...
  // Content management
  async getSizeGuidesWithContent(options?: SizeGuideQueryOptions): Promise<SizeGuide[]> {
  ...
  async getSizeGuidesWithoutContent(options?: SizeGuideQueryOptions): Promise<SizeGuide[]> {
    return await this.sizeGuideRepository.findAll({
      ...options,
      hasContent: false,
    });
  ...
  async updateSizeGuideContent(id: string, htmlContent: string): Promise<SizeGuide> {
  ...
  async getSizeGuideStats(): Promise<{
    ...
    const guidesByCategory = Array.from(categoryGroups.entries()).map(([category, count]) => ({
      category,
      count,
    }));

    const guidesWithContent = await this.sizeGuideRepository.count({ hasContent: true });
    const guidesWithoutContent = await this.sizeGuideRepository.count({ hasContent: false });
  ...
  // Validation methods
  async validateSizeGuideUniqueness(region: Region, category: string | null): Promise<boolean> {
    if (!category) return true; // General guides can have duplicates

    const existingGuide = await this.sizeGuideRepository.findByRegionAndCategory(region, category);
  ...
  private validateTitle(title: string): void {
    if (!title || title.trim().length === 0) {
      throw new Error('Size guide title cannot be empty');
    }

    if (title.trim().length > 200) {
      throw new Error('Size guide title cannot be longer than 200 characters');
    }
  }

  private validateHtmlContent(htmlContent: string): void {
    if (htmlContent.length > 50000) {
      throw new Error('Size guide content cannot exceed 50,000 characters');
    }
    ...
    while ((match = tagPattern.exec(htmlContent)) !== null) {
      ...
      if (tag.startsWith('/')) {
        ...
        if (lastOpenTag !== closingTag) {
          throw new Error(`Unmatched HTML tag: expected closing </${lastOpenTag}> but found </${closingTag}>`);
        }
