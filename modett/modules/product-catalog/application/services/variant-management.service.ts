import { IProductVariantRepository, VariantQueryOptions } from '../../domain/repositories/product-variant.repository';
import { IProductRepository } from '../../domain/repositories/product.repository';
import { ProductVariant, CreateVariantData } from '../../domain/entities/product-variant.entity';
...
export class VariantManagementService {
  ...
  async createVariant(productId: string, data: Omit<CreateVariantData, 'productId'>): Promise<ProductVariant> {
    ...
    if (data.sku) {
      const existingVariant = await this.productVariantRepository.findBySku(SKU.fromString(data.sku));
      if (existingVariant) {
  ...
  async getVariantsByProduct(productId: string, options: VariantServiceQueryOptions = {}): Promise<ProductVariant[]> {
    const {
      page = 1,
      limit = 20,
      size,
    ...
    variants.sort((a, b) => {
      ...
      switch (sortBy) {
        case 'price':
          ...
          break;
        case 'sku':
          comparison = a.getSku().getValue().localeCompare(b.getSku().getValue());
          break;
        case 'size':
  ...
  async getAllVariants(options: VariantServiceQueryOptions = {}): Promise<ProductVariant[]> {
    const {
      page = 1,
      limit = 50,
      sortBy = 'createdAt',
  ...
  async getVariantsByPriceRange(minPrice: number, maxPrice: number, options: VariantServiceQueryOptions = {}): Promise<ProductVariant[]> {
    const {
      page = 1,
      limit = 20,
      sortBy = 'price',
    ...
    return await this.productVariantRepository.findByPriceRange(minPrice, maxPrice, repositoryOptions);
  }

  async getVariantsBySize(size: string, options: VariantServiceQueryOptions = {}): Promise<ProductVariant[]> {
    const {
      page = 1,
      limit = 20,
      sortBy = 'price',
    ...
    return await this.productVariantRepository.findBySize(size, repositoryOptions);
  }

  async getVariantsByColor(color: string, options: VariantServiceQueryOptions = {}): Promise<ProductVariant[]> {
    const {
      page = 1,
      limit = 20,
      sortBy = 'price',
    ...
    return await this.productVariantRepository.findByColor(color, repositoryOptions);
  }

  async updateVariant(id: string, updateData: Partial<CreateVariantData>): Promise<ProductVariant | null> {
    const variantId = VariantId.fromString(id);
  ...
  async duplicateVariant(id: string, newSku: string): Promise<ProductVariant> {
    const variantId = VariantId.fromString(id);
    const originalVariant = await this.productVariantRepository.findById(variantId);
    ...
    const existingVariant = await this.productVariantRepository.findBySku(SKU.fromString(newSku));
    if (existingVariant) {
      throw new Error('Variant with this SKU already exists');
    }
    ...
    const duplicateData: CreateVariantData = {
      ...
      compareAtPrice: originalVariant.getCompareAtPrice()?.getValue() || undefined,
  ...
  async validateVariant(id: string): Promise<{
    ...
    if (compareAtPrice && compareAtPrice.getValue() <= variant.getPrice().getValue()) {
    ...
    const productExists = await this.productRepository.exists(variant.getProductId());
    if (!productExists) {
