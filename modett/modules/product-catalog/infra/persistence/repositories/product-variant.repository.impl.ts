...
import { IProductVariantRepository, VariantQueryOptions, VariantCountOptions } from "../../../domain/repositories/product-variant.repository";
...
export class ProductVariantRepository implements IProductVariantRepository {
  ...
  async save(variant: ProductVariant): Promise<void> {
    ...
    await this.prisma.productVariant.create({
      data: {
        ...
        compareAtPrice: data.compare_at_price ? parseFloat(data.compare_at_price) : null,
  ...
  async findById(id: VariantId): Promise<ProductVariant | null> {
    ...
    return ProductVariant.fromDatabaseRow({
      ...
      compare_at_price: variantData.compareAtPrice?.toString() || null,
  ...
  async findByProductId(productId: ProductId): Promise<ProductVariant[]> {
    ...
    return variants.map(variantData => ProductVariant.fromDatabaseRow({
      variant_id: variantData.id,
      product_id: variantData.productId,
      sku: variantData.sku,
      size: variantData.size,
  ...
  async findByPriceRange(min: number, max: number, options?: VariantQueryOptions): Promise<ProductVariant[]> {
    const {
      limit = 50,
      offset = 0,
      sortBy = 'price',
  ...
  async findBySize(size: string, options?: VariantQueryOptions): Promise<ProductVariant[]> {
    const {
      limit = 50,
      offset = 0,
      sortBy = 'createdAt',
  ...
  async findByColor(color: string, options?: VariantQueryOptions): Promise<ProductVariant[]> {
    const {
      limit = 50,
      offset = 0,
      sortBy = 'createdAt',
