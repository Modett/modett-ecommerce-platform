...
interface UpdateMediaAssetRequest extends Partial<Omit<CreateMediaAssetRequest, 'storageKey'>> {}
...
export class MediaController {
  ...
  async getMediaAssets(request: FastifyRequest<{ Querystring: MediaAssetQueryParams }>, reply: FastifyReply) {
    try {
      const {
        page = 1,
        limit = 20,
  ...
  async getMediaAsset(request: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply) {
    try {
  ...
  async createMediaAsset(request: FastifyRequest<{ Body: CreateMediaAssetRequest }>, reply: FastifyReply) {
    try {
      const assetData = request.body;

      // Basic validation
      if (!assetData.storageKey || typeof assetData.storageKey !== 'string' || assetData.storageKey.trim().length === 0) {
        return reply.code(400).send({
          success: false,
          error: 'Bad Request',
      ...
      if (!assetData.mime || typeof assetData.mime !== 'string' || assetData.mime.trim().length === 0) {
        return reply.code(400).send({
          success: false,
          error: 'Bad Request',
  ...
  async updateMediaAsset(request: FastifyRequest<{ Params: { id: string }; Body: UpdateMediaAssetRequest }>, reply: FastifyReply) {
    try {
      const { id } = request.params;
  ...
  async deleteMediaAsset(request: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply) {
    try {
      const { id } = request.params;
