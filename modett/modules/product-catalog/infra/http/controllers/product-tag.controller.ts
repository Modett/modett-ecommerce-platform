...
export class ProductTagController {
  ...
  async getTags(request: FastifyRequest<{ Querystring: TagQueryParams }>, reply: FastifyReply) {
    try {
      const {
        page = 1,
        limit = 20,
  ...
  async getTagByName(request: FastifyRequest<{ Params: { name: string } }>, reply: FastifyReply) {
    try {
      ...
      const tag = await this.productTagManagementService.getTagByName(decodeURIComponent(name));
  ...
  async createTag(request: FastifyRequest<{ Body: CreateTagRequest }>, reply: FastifyReply) {
    try {
      ...
      // Basic validation
      if (!tagData.tag || typeof tagData.tag !== 'string' || tagData.tag.trim().length === 0) {
        return reply.code(400).send({
          success: false,
          error: 'Bad Request',
          code: 'VALIDATION_ERROR',
  ...
  async updateTag(request: FastifyRequest<{ Params: { id: string }; Body: UpdateTagRequest }>, reply: FastifyReply) {
    try {
      const { id } = request.params;
      ...
      if (updateData.tag !== undefined) {
        if (typeof updateData.tag !== 'string' || updateData.tag.trim().length === 0) {
          return reply.code(400).send({
            success: false,
            error: 'Bad Request',
      ...
      const tag = await this.productTagManagementService.updateTag(id, updateData);
  ...
  async getTagSuggestions(request: FastifyRequest<{ Querystring: { query: string; limit?: number } }>, reply: FastifyReply) {
    try {
      ...
      const suggestions = await this.productTagManagementService.getTagSuggestions(
        query.trim(),
  ...
  async getMostUsedTags(request: FastifyRequest<{ Querystring: { limit?: number } }>, reply: FastifyReply) {
    try {
  ...
  async createBulkTags(request: FastifyRequest<{ Body: BulkCreateTagsRequest }>, reply: FastifyReply) {
    try {
  ...
  async deleteBulkTags(request: FastifyRequest<{ Body: BulkDeleteTagsRequest }>, reply: FastifyReply) {
    try {
      ...
      const result = await this.productTagManagementService.deleteMultipleTags(ids);
  ...
  async validateTag(request: FastifyRequest<{ Params: { name: string } }>, reply: FastifyReply) {
    try {
      const { name } = request.params;
      ...
      const isValid = await this.productTagManagementService.validateTag(decodeURIComponent(name));
