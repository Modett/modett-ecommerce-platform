...
export class CategoryController {
  ...
  async getCategories(request: FastifyRequest<{ Querystring: CategoryQueryParams }>, reply: FastifyReply) {
    try {
      const {
        page = 1,
        limit = 50,
  ...
  async getCategory(request: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply) {
    try {
  ...
  async getCategoryBySlug(request: FastifyRequest<{ Params: { slug: string } }>, reply: FastifyReply) {
    try {
  ...
  async createCategory(request: FastifyRequest<{ Body: CreateCategoryRequest }>, reply: FastifyReply) {
    try {
      ...
      if (categoryData.position !== undefined && (!Number.isInteger(categoryData.position) || categoryData.position < 0)) {
        return reply.code(400).send({
          success: false,
          error: 'Bad Request',
  ...
  async updateCategory(request: FastifyRequest<{ Params: { id: string }; Body: UpdateCategoryRequest }>, reply: FastifyReply) {
    try {
      const { id } = request.params;
      ...
      // Validate parentId if provided
      if (updateData.parentId !== undefined && typeof updateData.parentId !== 'string') {
        return reply.code(400).send({
          success: false,
          error: 'Bad Request',
          message: 'Parent ID must be a valid string'
      ...
      // Validate position if provided
      if (updateData.position !== undefined && (!Number.isInteger(updateData.position) || updateData.position < 0)) {
        return reply.code(400).send({
          success: false,
          error: 'Bad Request',
          message: 'Position must be a non-negative integer'
      ...
      if (error instanceof Error && (error.message.includes('parent') || error.message.includes('circular') || error.message.includes('hierarchy'))) {
        return reply.code(400).send({
          success: false,
          error: 'Bad Request',
          message: 'Invalid category hierarchy - would create circular reference or parent not found'
        });
      }
  ...
  async deleteCategory(request: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply) {
    try {
      ...
      if (error instanceof Error && (error.message.includes('children') || error.message.includes('constraint'))) {
        return reply.code(409).send({
          success: false,
          error: 'Conflict',
  ...
  async reorderCategories(request: FastifyRequest<{ Body: { categoryOrders: Array<{ id: string; position: number }> } }>, reply: FastifyReply) {
    try {
      ...
      for (const order of categoryOrders) {
        ...
        if (!Number.isInteger(order.position) || order.position < 0) {
          return reply.code(400).send({
            success: false,
            error: 'Bad Request',
            message: 'Each category order must have a valid position (non-negative integer)'
          });
        }
      }
