...
import { Region, CreateSizeGuideData } from '../../../domain/entities/size-guide.entity';
...
interface RegionalSizeGuideRequest extends Omit<CreateSizeGuideRequest, 'region'> {}

interface CategorySizeGuideRequest extends Omit<CreateSizeGuideRequest, 'category' | 'region'> {}
...
export class SizeGuideController {
  ...
  async getSizeGuides(request: FastifyRequest<{ Querystring: SizeGuideQueryParams }>, reply: FastifyReply) {
    try {
      const {
        page = 1,
        limit = 20,
      ...
      if (region && category) {
        guides = [await this.sizeGuideManagementService.getSizeGuideByRegionAndCategory(region, category)].filter(Boolean);
      } else if (region) {
        guides = await this.sizeGuideManagementService.getSizeGuidesByRegion(region, serviceOptions);
      } else if (category) {
        guides = await this.sizeGuideManagementService.getSizeGuidesByCategory(category, serviceOptions);
  ...
  async getSizeGuide(request: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply) {
    try {
  ...
  async createSizeGuide(request: FastifyRequest<{ Body: CreateSizeGuideRequest }>, reply: FastifyReply) {
    try {
      const guideData = request.body;

      // Basic validation
      if (!guideData.title || typeof guideData.title !== 'string' || guideData.title.trim().length === 0) {
        return reply.code(400).send({
          success: false,
          error: 'Bad Request',
      ...
      if (!guideData.region || !Object.values(Region).includes(guideData.region)) {
        return reply.code(400).send({
          success: false,
          error: 'Bad Request',
      ...
      const guide = await this.sizeGuideManagementService.createSizeGuide(guideData as CreateSizeGuideData);
  ...
  async updateSizeGuide(request: FastifyRequest<{ Params: { id: string }; Body: UpdateSizeGuideRequest }>, reply: FastifyReply) {
    try {
      const { id } = request.params;
      ...
      if (updateData.region && !Object.values(Region).includes(updateData.region)) {
        return reply.code(400).send({
          success: false,
          error: 'Bad Request',
      ...
      if (updateData.bodyHtml !== undefined && updateData.bodyHtml && updateData.bodyHtml.length > 50000) {
        return reply.code(400).send({
          success: false,
          error: 'Bad Request',
          message: 'HTML content cannot exceed 50,000 characters'
  ...
  async deleteSizeGuide(request: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply) {
    try {
      const { id } = request.params;
  ...
  async getRegionalSizeGuides(request: FastifyRequest<{ Params: { region: string }; Querystring: Omit<SizeGuideQueryParams, 'region'> }>, reply: FastifyReply) {
    try {
      const { region } = request.params;
      const { page = 1, limit = 20, category, hasContent, sortBy = 'title', sortOrder = 'desc' } = request.query;
      ...
      let guides;
      if (category) {
        const guide = await this.sizeGuideManagementService.getSizeGuideByRegionAndCategory(region as Region, category);
        guides = guide ? [guide] : [];
      } else {
  ...
  async createRegionalSizeGuide(request: FastifyRequest<{ Params: { region: string }; Body: RegionalSizeGuideRequest }>, reply: FastifyReply) {
    try {
      const { region } = request.params;
      const guideData = request.body;
      ...
      const guide = await this.sizeGuideManagementService.createRegionalSizeGuide(region as Region, guideData);
  ...
  async getGeneralSizeGuides(request: FastifyRequest<{ Params: { region: string } }>, reply: FastifyReply) {
    try {
      const { region } = request.params;
      ...
      const guides = await this.sizeGuideManagementService.getGeneralSizeGuides(region as Region);
  ...
  async createCategorySizeGuide(request: FastifyRequest<{ Params: { category: string; region: string }; Body: CategorySizeGuideRequest }>, reply: FastifyReply) {
    try {
      ...
      const guide = await this.sizeGuideManagementService.createCategorySizeGuide(
  ...
  async updateSizeGuideContent(request: FastifyRequest<{ Params: { id: string }; Body: { htmlContent: string } }>, reply: FastifyReply) {
    try {
      const { id } = request.params;
      ...
      const guide = await this.sizeGuideManagementService.updateSizeGuideContent(id, htmlContent);
  ...
  async clearSizeGuideContent(request: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply) {
    try {
      const { id } = request.params;
      ...
      const guide = await this.sizeGuideManagementService.clearSizeGuideContent(id);
  ...
  async getAvailableRegions(request: FastifyRequest, reply: FastifyReply) {
    try {
      const regions = await this.sizeGuideManagementService.getAvailableRegions();
  ...
  async getAvailableCategories(request: FastifyRequest<{ Querystring: { region?: string } }>, reply: FastifyReply) {
    try {
      ...
      const categories = await this.sizeGuideManagementService.getAvailableCategories(region as Region);
  ...
  async createBulkSizeGuides(request: FastifyRequest<{ Body: BulkCreateSizeGuidesRequest }>, reply: FastifyReply) {
    try {
  ...
  async deleteBulkSizeGuides(request: FastifyRequest<{ Body: BulkDeleteSizeGuidesRequest }>, reply: FastifyReply) {
    try {
      ...
      const result = await this.sizeGuideManagementService.deleteMultipleSizeGuides(ids);
  ...
  async validateUniqueness(request: FastifyRequest<{ Querystring: { region: string; category?: string } }>, reply: FastifyReply) {
    try {
      ...
      const isUnique = await this.sizeGuideManagementService.validateSizeGuideUniqueness(
        region as Region,
        category || null
      );
